...
root.configure(bg="#2C2F33")  # Background color of the board
...
def reset():
    ...
    for button in buttons:
        button.config(text=" ", state=NORMAL, bg="#b2eee6", fg="#23272A")  # Background of Buttons
...
def remove_oldest_move(moves):
    if len(moves) > 3:
        ...
        oldest_move.config(text=" ", bg="#b2eee6")  # Background of Buttons
...
def highlight_opponent_winning_move():
    ...
    def check_potential_win(a, b, c):
        if a["text"] == " " and b["text"] == opponent and c["text"] == opponent:
            a.config(bg="#8ad6cc")  # Highlight Possible Winning Move
        elif a["text"] == opponent and b["text"] == " " and c["text"] == opponent:
            b.config(bg="#8ad6cc")  # Highlight Possible Winning Move
        elif a["text"] == opponent and b["text"] == opponent and c["text"] == " ":
            c.config(bg="#8ad6cc")  # Highlight Possible Winning Move

    # Check all possible win conditions for opponent's
